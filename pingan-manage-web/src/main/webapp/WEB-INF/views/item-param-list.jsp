...
<table class="easyui-datagrid" id="itemParamList" title="产品列表" 
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'itemCatId',width:80">产品类目ID</th>
        	<th data-options="field:'itemCatName',width:100">产品类目</th>
...
<script>
    ...
    var itemParamListToolbar = [{
        ...
        handler:function(){
        	...
        	if(ids.length == 0){
        		$.messager.alert('提示','未选中产品规格!');
        		return ;
        	}
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的产品规格吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/rest/item/param/delete",params, function(data){
            			if(data.status == 200){
            				$.messager.alert('提示','删除产品规格成功!',undefined,function(){
